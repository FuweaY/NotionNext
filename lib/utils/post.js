...
export function getRecommendPost(post, allPosts, count = 6) {
  ...
  for (let i = 0; i < allPosts.length; i++) {
    ...
    for (let j = 0; j < currentTags.length; j++) {
      ...
      if (p.id && postIds.indexOf(p.id) > -1) {
        continue
      }
...
export async function processPostData(props, from) {
  ...
  // 文章内容加载
  if (!props.post.blockMap && props.post.id) {
    props.post.blockMap = await getPostBlocks(props.post.id, from)
  }

  if (props.post?.blockMap?.block && props.post?.id) {
    // 目录默认加载
